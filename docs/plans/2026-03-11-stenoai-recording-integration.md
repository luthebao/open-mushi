...
- Track B Milestone 1 tightened: RootActor lifecycle transitions are now guarded by an explicit allowlist (strict path + failure exits), duplicate-start blocking is covered for `Starting|Recording|Stopping` and active supervisor presence, and stale-state clear semantics are verified idempotent with status refresh emitted on every invocation.
- Track B Milestone 2 complete/tightened: STT queue output is now persisted to fs-db transcript artifacts before LLM stage (state sequencing remains `Transcribing -> QueuedForLlm -> Summarizing`), with consistent word timestamp/speaker-hint mapping into persisted transcript rows and explicit recording diagnostics for no-speech and low-confidence outcomes.
- Track B Milestone 5 complete: graph generation now auto-seeds from the just-completed session when recording transitions to `Completed`, using persisted summary + transcript artifacts (including timestamp/speaker metadata availability); manual graph generation remains available as fallback, and graph node detail rows now show provenance labels (`from summary/transcript` + metadata badges).
- Track B Milestone 6 complete: preflight checks now gate recording starts (backend preflight as source of truth), listen controls surface actionable remediation for model/device/permission failures, stale-state recovery is exposed as an explicit `Recover` action when failures indicate stale artifacts, and queue/stage visibility is shown in both header status chip diagnostics and sidebar pipeline indicators.
- 2026-03-12 final hardening fixes: `start_session` now propagates root actor start rejection as an explicit command error (frontend no longer flips `live.status` to active optimistically and waits for lifecycle/status confirmation), queue/post-processing states no longer block new recording starts when lifecycle is inactive, and graph auto-seed now keys off durable listener store marker `lastCompletedSessionId` to avoid transient `completed` race windows.
