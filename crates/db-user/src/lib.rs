...
pub use openmushi_db_core::{Database, Error};
...
macro_rules! user_common_derives {
    (#[sql_table($table:expr)] $(#[$meta:meta])* $vis:vis $kind:ident $name:ident {
        ...
        impl openmushi_db_core::SqlTable for $name {
...
pub struct UserDatabase {
    db: openmushi_db_core::Database,
}

impl UserDatabase {
    pub fn from(db: openmushi_db_core::Database) -> Self {
        Self { db }
    }
}

impl std::ops::Deref for UserDatabase {
    type Target = openmushi_db_core::Database;
...
pub async fn migrate(db: &UserDatabase) -> Result<(), crate::Error> {
    let conn = db.conn()?;
    openmushi_db_core::migrate(&conn, MIGRATIONS.to_vec()).await?;
...
mod tests {
    ...
    use crate::migrate;
    use openmushi_db_core::DatabaseBuilder;
