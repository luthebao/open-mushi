...
// TODO: migrate existing "hyprnote.json" files to "openmushi.json"
pub const VAULT_CONFIG_FILENAME: &str = "openmushi.json";
...
fn resolve_app_folder(bundle_id: &str) -> &str {
    if cfg!(debug_assertions) || bundle_id == "com.openmushi.staging" {
        bundle_id
    } else {
        // TODO: migrate existing "hyprnote" folder to "openmushi"
        "openmushi"
    }
}
...
mod tests {
    ...
    fn resolve_app_folder_returns_bundle_id_for_staging() {
        assert_eq!(
            resolve_app_folder("com.openmushi.staging"),
