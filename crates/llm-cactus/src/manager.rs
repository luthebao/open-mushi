...
impl ModelLoader for openmushi_cactus::Model {
    fn load(path: &Path) -> Result<Self, crate::Error> {
        Ok(openmushi_cactus::Model::new(path)?)
    }
}
...
pub struct ModelManager<M: ModelLoader = openmushi_cactus::Model> {
...
pub struct ModelManagerBuilder<M: ModelLoader = openmushi_cactus::Model> {
