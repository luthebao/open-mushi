...
#[pin_project]
pub struct NormalizedSource<S: openmushi_audio_interface::AsyncSource> {
    source: S,
    gain_linear: f32,
    ebur128: EbuR128,
...
pub trait NormalizeExt<S: openmushi_audio_interface::AsyncSource> {
    fn normalize(self) -> NormalizedSource<S>;
}

impl<S: openmushi_audio_interface::AsyncSource> NormalizeExt<S> for S {
    fn normalize(self) -> NormalizedSource<S> {
...
impl<S: openmushi_audio_interface::AsyncSource> Stream for NormalizedSource<S> {
    type Item = f32;
...
impl<S: openmushi_audio_interface::AsyncSource> openmushi_audio_interface::AsyncSource
