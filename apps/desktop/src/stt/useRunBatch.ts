...
import type { BatchParams } from "~/store/zustand/listener/general";
import type { TranscriptStorage } from "@openmushi/store";
...
export const useRunBatch = (sessionId: string) => {
  ...
  const speakerEnabled = useConfigValue("speaker_diarization_enabled");
  const speakerModel = useConfigValue("speaker_diarization_model");
  const speakerThreshold = useConfigValue("speaker_similarity_threshold");
