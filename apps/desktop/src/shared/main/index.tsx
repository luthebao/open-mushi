...
import { commands as flagCommands } from "@openmushi/plugin-flag";
...
import { TabContentGraph, TabItemGraph } from "~/graph";
...
import { TabContentWorkspace, TabItemWorkspace } from "~/workspaces";
...
function Header({ tabs }: { tabs: Tab[] }) {
  ...
  const handleNewWorkspace = useNewWorkspace();
  const handleOpenGraph = useCallback(() => {
    const { openNew: _openNew } = useTabs.getState();
    _openNew({ type: "graph" });
  }, []);
  const showNewTabMenu = useNativeContextMenu([
    ...
    { separator: true },
    { id: "new-workspace", text: "Create Workspace", action: handleNewWorkspace },
    { id: "open-graph", text: "Knowledge Graph", action: handleOpenGraph },
  ]);
...
function ContentWrapper({ tab }: { tab: Tab }) {
  ...
  if (tab.type === "workspaces") {
    return <TabContentWorkspace tab={tab} />;
  }
  if (tab.type === "humans") {
  ...
  if (tab.type === "graph") {
    return <TabContentGraph tab={tab} />;
  }
  return null;
}
