...
import type { EventStorage } from "@openmushi/store";
...
const PROVIDERS: MeetingProvider[] = [
  {
    ...
    generateInvite: ({ title, host, meetingId, passcode, link }) => {
      ...
      const aiCompanionLink = `https://openmushi.${PROVIDERS[0].domain}/launch/edl?muid=${faker.string.uuid()}`;
      const joinInstructionsLink = `https://openmushi.${PROVIDERS[0].domain}/meetings/${meetingId.replace(
        /\s/g,
        "",
...
export const createEvent = (calendar_id: string) => {
  ...
  if (meetingType === "online" || meetingType === "hybrid") {
    meeting_link = `https://openmushi.${provider.domain}/${meetingPath}`;
