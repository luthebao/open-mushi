...
import type { AmModel, SupportedSttModel } from "@openmushi/plugin-local-stt";
...
export const displayModelId = (model: string) => {
  ...
  if (model.startsWith("sherpa-")) {
    const name = model.replace("sherpa-", "");
    return name.split("-").map(s => s.charAt(0).toUpperCase() + s.slice(1)).join(" ") + " (Local)";
  }
