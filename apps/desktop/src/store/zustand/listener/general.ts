...
import { commands as hooksCommands } from "@openmushi/plugin-hooks";
...
// BatchParams was from @openmushi/plugin-listener2 (removed).
// Define a compatible local type for batch processing parameters.
export type BatchParams = {
  ...
  speaker_model?: SpeakerModel | null;
...
>(
      ...
      const [dataDirPath, micUsingApps, bundleId] = yield* Effect.tryPromise({
        try: () =>
          Promise.all([
            ...
            getIdentifier().catch(() => "com.openmushi.stable"),
          ]),
        catch: (error) => error,
      });
  ...
  runBatch: async (params, options) => {
    ...
    if (params.provider === "sherpa") {
      get().handleBatchStarted(sessionId, "transcribing");

      try {
        // Use speaker settings passed from the React hook (read from TinyBase)
        // to avoid relying on Rust SharedState sync timing.
        let speakerModel: SpeakerModel | null = null;
        let speakerThreshold: number | null = null;

        if (params.speaker_enabled && params.speaker_model) {
          speakerModel = params.speaker_model;
          speakerThreshold = params.speaker_threshold ?? 0.5;
        }

        const result = await localSttCommands.runBatchSherpa(
          params.file_path,
          params.model as SherpaSttModel,
          params.languages[0] ?? null,
          speakerModel,
          speakerThreshold,
        );
        ...
        if (total === 0) {
          get().handleBatchFailed(sessionId, "No speech detected in audio file");
          return;
        }
        ...
        for (let i = 0; i < total; i++) {
          ...
          get().handleBatchResponseStreamed(
            sessionId,
            response as unknown as StreamResponse,
            percentage,
          );
        }
    ...
    get().handleBatchFailed(
      sessionId,
      `Batch processing not available for provider: ${params.provider}`,
    );
  },
