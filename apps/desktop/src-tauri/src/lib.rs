...
pub async fn main() {
    ...
    let app = builder
        ...
        .setup(move |app| {
            ...
            {
                use tauri_plugin_tray::AppMenuItem;
                app_handle.on_menu_event(|app, event| {
                    if let Ok(item) = AppMenuItem::try_from(event.id().clone()) {
                        item.handle(app);
                    }
                });
            }
    ...
    #[cfg(target_os = "macos")]
    openmushi_intercept::setup_force_quit_handler();

    #[cfg(target_os = "macos")]
    {
        let handle = app.handle().clone();
        openmushi_intercept::set_close_handler(move || {
    ...
    app.run(move |app, event| match event {
        ...
        tauri::RunEvent::ExitRequested { api, .. } => {
            if openmushi_intercept::should_force_quit() {
                return;
            }
...
fn make_specta_builder<R: tauri::Runtime>() -> tauri_specta::Builder<R> {
    tauri_specta::Builder::<R>::new()
        .commands(tauri_specta::collect_commands![
            ...
            commands::list_skills::<tauri::Wry>,
        ])
