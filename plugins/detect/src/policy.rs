...
use openmushi_notification_interface::NotificationKey;
...
impl AppCategory {
    pub fn bundle_ids(&self) -> &'static [&'static str] {
        match self {
            Self::OpenMushi => &[
                "com.openmushi.dev",
                "com.openmushi.stable",
                "com.openmushi.nightly",
                "com.openmushi.staging",
            ],
            Self::Dictation => &[
...
pub struct PolicyContext<'a> {
    pub apps: &'a [openmushi_detect::InstalledApp],
    pub is_dnd: bool,
...
pub struct PolicyResult {
    pub filtered_apps: Vec<openmushi_detect::InstalledApp>,
...
impl MicNotificationPolicy {
    ...
    fn filter_apps(
        &self,
        apps: &[openmushi_detect::InstalledApp],
        is_dnd: bool,
    ) -> Result<Vec<openmushi_detect::InstalledApp>, SkipReason> {
...
mod tests {
    ...
    fn app(id: &str) -> openmushi_detect::InstalledApp {
    ...
    fn test_app_category_find() {
        assert_eq!(
            AppCategory::find_category("com.openmushi.dev"),
            Some(AppCategory::OpenMushi)
        );
    ...
    fn test_app_category_all_returns_every_variant() {
        ...
        assert!(all.contains(&AppCategory::OpenMushi));
    ...
    fn test_should_not_track_categorized_app() {
        let policy = MicNotificationPolicy::default();
        assert!(!policy.should_track_app("com.openmushi.dev"));
    ...
    fn test_evaluate_filters_all_categorized_apps() {
        let policy = MicNotificationPolicy::default();
        let apps = vec![app("com.openmushi.dev"), app("com.electron.aqua-voice")];
        let ctx = PolicyContext {
            apps: &apps,
            is_dnd: false,
            event_type: MicEventType::Started,
    ...
    fn test_evaluate_empty_apps_list() {
        let policy = MicNotificationPolicy::default();
        let apps: Vec<openmushi_detect::InstalledApp> = vec![];
        let ctx = PolicyContext {
            apps: &apps,
            is_dnd: false,
            event_type: MicEventType::Started,
    ...
    fn test_policy_with_no_ignored_categories_passes_all() {
        ...
        let apps = vec![app("com.openmushi.dev"), app("us.zoom.xos")];
        let ctx = PolicyContext {
            apps: &apps,
            is_dnd: false,
            event_type: MicEventType::Started,
    ...
    fn test_policy_with_selective_ignored_categories() {
        ...
        let result = policy.evaluate(&ctx).unwrap();
        let ids: Vec<_> = result.filtered_apps.iter().map(|a| a.id.as_str()).collect();
        assert_eq!(ids, vec!["com.openmushi.dev", "us.zoom.xos"]);
    }
}
