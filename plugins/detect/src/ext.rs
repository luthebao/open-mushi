...
impl<'a, R: tauri::Runtime, M: tauri::Manager<R>> Detect<'a, R, M> {
    pub fn list_installed_applications(&self) -> Vec<openmushi_detect::InstalledApp> {
        openmushi_detect::list_installed_apps()
    }

    pub fn list_mic_using_applications(&self) -> Vec<openmushi_detect::InstalledApp> {
        openmushi_detect::list_mic_using_apps()
    }
