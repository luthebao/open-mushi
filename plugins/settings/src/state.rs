...
impl State {
    ...
    fn path(&self) -> PathBuf {
        openmushi_storage::vault::compute_settings_path(&self.vault_base)
    }
    ...
    pub async fn save(&self, settings: serde_json::Value) -> crate::Result<()> {
        ...
        openmushi_storage::fs::atomic_write_async(&self.path(), &content).await?;
        Ok(())
    }

    pub fn reset(&self) -> crate::Result<()> {
        openmushi_storage::fs::atomic_write(&self.path(), "{}")?;
        Ok(())
    }
}
