...
use openmushi_storage::ObsidianVault;
...
impl<'a, R: tauri::Runtime, M: tauri::Manager<R>> Settings<'a, R, M> {
    pub fn default_base(&self) -> Result<PathBuf, crate::Error> {
        ...
        let path = openmushi_storage::global::compute_default_base(bundle_id)
            .ok_or(openmushi_storage::Error::DataDirUnavailable)?;
    ...
    pub fn global_base(&self) -> Result<Utf8PathBuf, crate::Error> {
        ...
        Utf8PathBuf::from_path_buf(path).map_err(|_| openmushi_storage::Error::PathNotValidUtf8.into())
    }

    pub fn settings_path(&self) -> Result<Utf8PathBuf, crate::Error> {
        let base = self.cached_vault_base()?;
        Ok(base.join(openmushi_storage::vault::SETTINGS_FILENAME))
    }
    ...
    pub fn cached_vault_base(&self) -> Result<Utf8PathBuf, crate::Error> {
        ...
        Utf8PathBuf::from_path_buf(state.vault_base().clone())
            .map_err(|_| openmushi_storage::Error::PathNotValidUtf8.into())
    }
    ...
    pub fn fresh_vault_base(&self) -> Result<PathBuf, crate::Error> {
        ...
        let custom_base = openmushi_storage::vault::resolve_custom(global_base.as_ref(), &default_base);
        Ok(custom_base.unwrap_or(default_base))
    }

    pub fn obsidian_vaults(&self) -> Result<Vec<ObsidianVault>, crate::Error> {
        openmushi_storage::obsidian::list_vaults().map_err(Into::into)
    }
...
impl<'a, R: tauri::Runtime, M: tauri::Manager<R> + tauri::Emitter<R>> Settings<'a, R, M> {
    pub async fn change_vault_base(&self, new_path: Utf8PathBuf) -> Result<(), crate::Error> {
        ...
        openmushi_storage::vault::validate_vault_base_change(
            old_vault_base.as_ref(),
            new_path.as_ref(),
        )?;
        openmushi_storage::vault::ensure_vault_dir(new_path.as_ref())?;
        openmushi_storage::vault::copy_vault_items(old_vault_base.as_ref(), new_path.as_ref()).await?;

        let vault_config_path = openmushi_storage::global::compute_vault_config_path(&default_base);
        let mut config = std::fs::read_to_string(&vault_config_path)
            .ok()
            .and_then(|s| serde_json::from_str(&s).ok())
            .unwrap_or_else(|| serde_json::json!({}));
        openmushi_storage::vault::set_vault_path(&mut config, new_path.as_ref());

        openmushi_storage::fs::atomic_write(
            &vault_config_path,
