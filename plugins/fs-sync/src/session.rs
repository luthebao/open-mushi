...
mod tests {
    ...
    fn find_session_at_root() {
        ...
        let result = find_session_dir(&env.path().join("workspaces"), UUID_1);
        assert_eq!(result, env.folder_session_path("workspaces", UUID_1));
    }
    ...
    fn find_session_fallback_when_not_found() {
        ...
        let sessions = temp.child("workspaces");
