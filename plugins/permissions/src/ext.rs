...
impl<'a, R: tauri::Runtime, M: tauri::Manager<R>> Permissions<'a, R, M> {
    ...
    async fn check_microphone(&self) -> Result<PermissionStatus, crate::Error> {
        ...
        #[cfg(not(target_os = "macos"))]
        {
            ...
            let mut mic_sample_stream = openmushi_audio::AudioInput::from_mic(None)?.stream();
    ...
    async fn check_system_audio(&self) -> Result<PermissionStatus, crate::Error> {
        #[cfg(target_os = "macos")]
        return check!("system_audio", openmushi_tcc::audio_capture_permission_status());

        #[cfg(not(target_os = "macos"))]
        {
            use futures_util::StreamExt;
            let mut speaker_sample_stream = openmushi_audio::AudioInput::from_speaker().stream();
    ...
    async fn request_system_audio(&self) -> Result<(), crate::Error> {
        let stop = openmushi_audio::AudioOutput::silence();
    ...
    async fn reset_tcc(&self, service: &str) {
        ...
        let bundle_id = if cfg!(debug_assertions) {
            match openmushi_bundle::get_ancestor_bundle_id() {
                Some(id) => {
